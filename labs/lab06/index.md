...
Floating point arithmetic in bash is quite complicated, so try to use integers when possible -- you may want to use the `Timer::getTime()` method, which includes milliseconds in its return value.
