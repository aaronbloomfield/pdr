...
This conjecture has been proven for all integers up to at least 5.6 * 10^^13^^, but has not yet been proven for all (positive) integers.  It is widely believed to be true, however.  If you are interested, more information on this conjecture can be found at [http://en.wikipedia.org/wiki/Collatz_conjecture](http://en.wikipedia.org/wiki/Collatz_conjecture). 
...
1. Asks the user for an input value, n, which is the number of times to call the subroutine.
1. Runs the subroutine n times with the parameter x as the input.
